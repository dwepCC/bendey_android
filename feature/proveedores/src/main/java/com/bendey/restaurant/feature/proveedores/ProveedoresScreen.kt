...
/** Proveedores — mismo modelo de Contact que Clientes (type='supplier'). Se usan en Compras. */
...
fun ProveedoresScreen(
    ...
    BendeyListScreenLayout(
        ...
        header = {
            BendeyScreenToolbar(
                title = "Proveedores",
                subtitle = proveedoresSubtitle(state),
                actions = {
                    BendeyIconButton(
...
private fun proveedoresSubtitle(state: ProveedoresUiState): String {
    val count = state.contacts.size
    return when {
        state.showInactive -> if (count == 1) "1 proveedor" else "$count proveedores"
        count == 1 -> "1 proveedor activo"
        else -> "$count proveedores activos"
    }
}

@Composable
private fun ProveedoresListPane(
    state: ProveedoresUiState,
    onEdit: (Int) -> Unit,
    onDelete: (Int) -> Unit,
    onToggle: (Int) -> Unit,
...
private fun ProveedorRow(
    ...
    BendeyCard(
        ...
        Row(
            ...
            BendeyIconButton(onClick = onEdit, icon = Icons.Default.Edit, contentDescription = "Editar")
            BendeyIconButton(
                onClick = onDelete,
...
private fun ProveedorFormDialog(
    ...
    BendeyFormDialog(
        ...
        title = if (isEditing) "Editar proveedor" else "Nuevo proveedor",
